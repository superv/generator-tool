...
//            $args = [$fieldName, $fieldType];
        ...
        $tableArray = [];
        $relations = [];
        foreach (array_merge($fields, $relations, $indexes) as $item) {
            $tableArray[] = $this->getItem($item, 'resource');
        }
    ...
    {
        $config = array_only($this->data, ['nav', 'label']);
        ...
        foreach (array_filter($config) as $key => $value) {
            ...
            $configArray[] = $this->getItem($item, 'resource');
        }
